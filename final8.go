...
// Função separada para calcular o sinal (+1 ou -1)
...
func determinanteBaseline(mat [][]int) int {
	...
	det = 0
	for j = 0; j < ordem; j++ {
		det = det + sinal(0, j)*mat[0][j]*determinanteBaseline(menorMatriz(mat, 0, j))
	}
	return det
}
	...
	if maiorZerosLinha >= maiorZerosColuna {
		for j = 0; j < ordem; j++ {
			if mat[linhaEscolhida][j] != 0 {
				...
				det = det + sinal(linhaEscolhida, j)*mat[linhaEscolhida][j]*determinanteOtimizado(submat)
			}
		}
	} else {
		for i = 0; i < ordem; i++ {
			if mat[i][colunaEscolhida] != 0 {
				...
				det = det + sinal(i, colunaEscolhida)*mat[i][colunaEscolhida]*determinanteOtimizado(submat)
			}
		}
	}
	return det
